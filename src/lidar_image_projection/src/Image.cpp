...
Mat Image::computeSobelEdgeImage()
{
  ...
  if (this->img.channels() > 1)
  {
    ...
    cvtColor(this->img, grayImg, COLOR_BGR2GRAY);
  }
  else
  {
...
bool Image::detect4Circles(float canny_thresh, float center_thresh, vector<Point2f> &centers, vector<float> &radiuses)
{
  ...
  {
    HoughCircles(src_gray, circles, HOUGH_GRADIENT, 1, src_gray.rows / 8, canny_thresh, thresh, 0, 0);
  }
  ...
  /// Draw the circles detected
   ...
   Mat src_rgb;
   cvtColor(img, src_rgb, COLOR_GRAY2BGR );
...
Mat Image::segmentation(int segment_count)
{
  ...
  Mat segmentation;
  adaptiveThreshold(img, segmentation, 1, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 401, 2);
  return segmentation;
}
