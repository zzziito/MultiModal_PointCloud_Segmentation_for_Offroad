...
//        addGaussianNoise(C_T_L);
    ...
    void colorPointCloud() {
        ...
        ROS_INFO_STREAM("objectPoints_L Size " << objectPoints_L.size());

        for(size_t i = 0; i < objectPoints_L.size(); i++) {
            ROS_INFO_STREAM("inside colorPointCloud" );
    ...
    void colorLidarPointsOnImage(double min_range,
        ...
        double error = 0;
        double count = 0;
        ROS_INFO_STREAM("img Point Size " << imagePoints.size());
...
//
        ...
        // Project the Lidar points onto the image
        cv::projectPoints(objectPoints_L, rvec, tvec, projection_matrix, distCoeff, imagePoints);

        // Publish the image with projected points (optional)
        // if (color_projection) {
        //     image_in = cv_ptr->image.clone();
        //     image_out = image_in.clone();
        // } else {
        //     image_in = cv_ptr->image.clone();
        //     cv::cvtColor(image_in, image_out, cv::COLOR_GRAY2BGR);
        // }

        // out_cloud_pcl.header = cloud_msg->header;


        pcl::PCLPointCloud2 pcl_pc2;
        pcl_conversions::toPCL(*cloud_msg, pcl_pc2);
        pcl::fromPCLPointCloud2(pcl_pc2, out_cloud_pcl);
        out_cloud_pcl.header = pcl_conversions::toPCL(cloud_msg->header);

        out_cloud_pcl.is_dense = false;
        out_cloud_pcl.points.clear();

        pcl::PointXYZRGB p;

        // New point cloud to store points within the camera angle
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr filtered_cloud(new pcl::PointCloud<pcl::PointXYZRGB>);

        pcl::PCLPointCloud2 pcl_pc3;
        pcl_conversions::toPCL(*cloud_msg, pcl_pc3);
        pcl::fromPCLPointCloud2(pcl_pc3, filtered_cloud);
        ...
        for (size_t i = 0; i < imagePoints.size(); ++i) {
            ...
            if (imagePoints[i].x >= 0 && imagePoints[i].x < cv_ptr->image.cols &&
                imagePoints[i].y >= 0 && imagePoints[i].y < cv_ptr->image.rows) {
                p.x = cloud->points[i].x;
                p.y = cloud->points[i].y;
                p.z = cloud->points[i].z;

                // Check if the point lies within the camera angle
                if (checkCameraAngle(imagePoints[i], cv_ptr->image.cols, cv_ptr->image.rows)) {
                    if (color_projection) {
                        p.r = image_in.at<cv::Vec3b>(imagePoints[i].y, imagePoints[i].x)[2];
                        p.g = image_in.at<cv::Vec3b>(imagePoints[i].y, imagePoints[i].x)[1];
                        p.b = image_in.at<cv::Vec3b>(imagePoints[i].y, imagePoints[i].x)[0];
                    } else {
                        p.r = 255;
                        p.g = 255;
                        p.b = 255;
                    }
                    out_cloud_pcl.points.push_back(p);

                    // Add the point to the filtered cloud
                    filtered_cloud->points.push_back(pcl::PointXYZRGB(p));
                }
            }
        }

        // Publish the new point cloud containing only points within the camera angle
        ...
        cloud_pub.publish(filtered_cloud_ros);
...
//
        ...
        for(size_t i = 0; i < in_cloud->points.size(); i++) {
                // ROS_INFO_STREAM(" inside for " << in_cloud->points.size());
